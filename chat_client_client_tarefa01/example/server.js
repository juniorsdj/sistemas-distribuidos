...
function Message(type, message, rinfo) {
	...
	this.typeConnect = function () {
		...
		var _message = util.format('Nova conexão: ', this.rinfo.port);
		clients.push(rinfo);
	...
	this.typeDisconnect = function () {
		...
		var _message = util.format('Desconectado: ', this.rinfo.port);
...
var server = dgram.createSocket('udp4', function (data, rinfo) {
	...
	process.stdin.on('data', function (chunk) {
		...
		var buffer =  Buffer.from('Server => %s', chunk.toString().replace(/\n|\n/g, ''));
